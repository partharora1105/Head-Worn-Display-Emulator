...
namespace Microsoft.Samples.Kinect.FaceBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        public MainWindow()
        {
            ...
            this.yawList = new List<int>();
            this.rollList = new List<int>();
            this.pitchList = new List<int>();
        ...
        private void DrawFaceFrameResults(int faceIndex, FaceFrameResult faceResult, DrawingContext drawingContext)
        {
            ...
            {
                const int maxAverageFrameRange = 25;
                int pitch, yaw, roll;
                ExtractFaceRotationInDegrees(faceResult.FaceRotationQuaternion, out pitch, out yaw, out roll);
                if (pitchList.Count + 1 > maxAverageFrameRange) {
                ...
                for (int i = 0; i < pitchList.Count; i++) {
                    sumPitch += pitchList[i];
                    sumYaw += yawList[i];
                    sumRoll += rollList[i];
                }
                pitch = sumPitch / pitchList.Count;
                yaw = sumYaw / yawList.Count;
                roll = sumRoll / rollList.Count;
